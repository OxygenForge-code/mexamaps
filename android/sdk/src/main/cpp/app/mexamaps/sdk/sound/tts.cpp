...
extern "C"
{
JNIEXPORT void Java_app_mexamaps_sdk_sound_TtsPlayer_nativeEnableTurnNotifications(JNIEnv *, jclass, jboolean enable)
{
  return frm()->GetRoutingManager().EnableTurnNotifications(static_cast<bool>(enable));
}

JNIEXPORT jboolean Java_app_mexamaps_sdk_sound_TtsPlayer_nativeAreTurnNotificationsEnabled(JNIEnv *, jclass)
{
  return static_cast<jboolean>(frm()->GetRoutingManager().AreTurnNotificationsEnabled());
}

JNIEXPORT void Java_app_mexamaps_sdk_sound_TtsPlayer_nativeSetTurnNotificationsLocale(JNIEnv * env, jclass,
                                                                                         jstring jLocale)
{
  frm()->GetRoutingManager().SetTurnNotificationsLocale(jni::ToNativeString(env, jLocale));
}

JNIEXPORT jstring Java_app_mexamaps_sdk_sound_TtsPlayer_nativeGetTurnNotificationsLocale(JNIEnv * env, jclass)
{
  return jni::ToJavaString(env, frm()->GetRoutingManager().GetTurnNotificationsLocale());
}

JNIEXPORT jobject Java_app_mexamaps_sdk_sound_TtsPlayer_nativeGetSupportedLanguages(JNIEnv * env, jclass)
{
