package app.mexamaps.util;

import static app.mexamaps.sdk.util.Utils.dimen;
import static app.mexamaps.sdk.util.Utils.isIntentSupported;
...
import app.mexamaps.BuildConfig;
...
public class Utils
{
  ...
  public static void copyTextToClipboard(Context context, String text)
  {
    ...
    final ClipData clip = ClipData.newPlainText("MexaMaps: " + text, text);
  ...
  {
    subject = "MexaMaps Bugreport" + (TextUtils.isEmpty(subject) ? "" : ": " + subject);
  ...
  {
    LogsManager.INSTANCE.zipLogs(
        new SupportInfoWithLogsCallback(launcher, activity, "MexaMaps Feedback", "", BuildConfig.SUPPORT_MAIL));
  }
  ...
  {
    if (MwmApplication.from(context).getMexaMaps().arePlatformAndCoreInitialized())
      return;
  ...
  public static String getDonateUrl(@NonNull Context context)
  {
    ...
    // Enable donations by default if not Google or Huawei. Replace mexamaps.app/donate/ with localized page.
    if ((url.isEmpty() && !BuildConfig.FLAVOR.equals("google") && !BuildConfig.FLAVOR.equals("huawei"))
        || url.endsWith("mexamaps.app/donate/"))
